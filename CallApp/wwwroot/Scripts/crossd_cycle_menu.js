function initializeMenues(userid, objectid, isplanmenu) {
        // общее для плана-меню и цикличного
        ...
        window.generateMenuCategoryTimeRecipe = function (id, eatingID, orderNumber) {
            ...
            categoryRow["ControlPortionCount"] = $("#ControlPortionInput").val();
            categoryRow["PortionCountFact"] = $("#PortionCountFact").val();
            categoryRow["PersonPortionCount"] = $("#PersonPortionCount").val();
        ...
        // открытие модального окна для замены блюда
       /* $("#menuCategoryTimeRecipeGrid").bind("celldoubleclick", function (event) {
            // модальное окно открывается только при двойном клике на ячейке с  названием рецепта
            if (event.args.datafield == "RecipeID") {
                createModalToChangeRecipe(event.args.row.bounddata);
            }
        });*/
        ...
        window.records = menuTreeAdapter.getRecordsHierarchy("id", "parentid", "items", [{ name: "text", map: "label" }]);

        $("#menuTree").jqxTree({ source: records, width: "100%", height: "100%"});     
        if (isplanmenu) {
        ...
        window.refreshTree = function() {
            ...
            menuTreeAdapter.dataBind();
            records = menuTreeAdapter.getRecordsHierarchy("id", "parentid", "items", [{ name: "text", map: "label" }]);
        ...
        $("#splitter").jqxSplitter({ width: 1130, height:1000, panels: [{ size: 200 }, { size: 930 }] });
        ...
        window.menuCategoryTimeRecipeSource =
        {
            ...
            beforeprocessing: function (data) {
                ...
                // для правильной сортировки основных блюд вместе с заменами для аллергиков
                //data.forEach(item => {
                //    if(item.ParentMenuCategoryTimeRecipeID) {
                //        item.sortColumn = item.EatingTimeID + "." +  item.ParentMenuCategoryTimeRecipeID + "." + item.MenuCategoryTimeRecipeID;
                //    } else {
                //        item.sortColumn = item.EatingTimeID + "." + item.MenuCategoryTimeRecipeID;
                //    }
                //});

                //data.sort(function(itemA, itemB) {
                //    var r = 0;
                //    if (itemA.sortColumn > itemB.sortColumn) r = 1;
                //    if (itemA.sortColumn < itemB.sortColumn) r = -1;
                //    return r;
                //});
            },
        ...
        if (isplanmenu) { recipeNameWidth = 260 };
        ...
        var gridColumns = [
                ...
                },
                {
                    ...
                    createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
                        ...
                        editor.jqxComboBox({ source: new $.jqx.dataAdapter(recipeDS), valueMember: "RecipeID", displayMember: "Name", width: recipeNameWidth, dropDownWidth: recipeNameWidth });
                    },
                ...
                //{
                //    text: "", datafield: "Edit", sortable: false, width: 20,
                //    createwidget: function (row, column, value, htmlElement) {
                //        var button = $("<div style='border:none;'><span class='glyphicon glyphicon-pencil pencil-icon' style='margin: auto'></span></div>");
                //        $(htmlElement).append(button);
                //        button.jqxButton({ height: '100%', width: '100%' });
                //        $(".pencil-icon").jqxTooltip({ content: '<b>Редактировать</b>', position: 'mouse', name: 'movieTooltip' });
                //        button.click(function () {
                //            createNewRecipe(row);
                //        });
                //    },
                //    initwidget: function (row, column, value, htmlElement) {
                //        $(htmlElement).find('.pencil-icon')[0].style.marginTop = "4px";
                //        $(htmlElement).find('.pencil-icon')[0].style.marginRight = "4px";
                //    }
                //},
                {
                    text: "Выход", datafield: "Netto", editable: true, cellsformat: "d3", cellsalign: "right", width: 90,
                    cellclassname: formatRecordStatus
                },
                {
                    text: "", datafield: "Delete", sortable: false, width: 20,
        ...
        if (isplanmenu) {
            ...
            gridColumns.splice(5, 0,
                ...
                {
                    text: "Персонал", datafield: "PersonPortionCount", width: 80
                },
                 {
        ...
        {
            width: categoryTimeRecipeGridWidth,
            columnsheight: 40,
            columnsresize: true,
            autoheight: true,
            editable: true,
            ...
            //    var cell = $('#menuCategoryTimeRecipeGrid').jqxGrid('getcellatposition', pageX, pageY);

            //    if (cell.column === 'RecipeID') {
            //        countTooltip++;

            //        if (countTooltip == 1) {
            //            var dummy = document.createElement('div');
            //            dummy.className = 'jqx-grid-cell jqx-item';
            //            dummy.style.position = 'absolute';
            //            dummy.style.visibility = 'hidden';
            //            var text = $('#menuCategoryTimeRecipeGrid').jqxGrid('getrowdata', cell.row).RecipeName
            //            dummy.innerHTML = text;

            //            $('#menuCategoryTimeRecipeGrid').append(dummy);

            //            var dummyWidth = $(dummy).width();

            //            $(dummy).remove();

            //            if (dummyWidth > recipeNameWidth - 10) {
            //                // update tooltip.
            //                $("#menuCategoryTimeRecipeGrid").jqxTooltip({ content: text });
            //                // open tooltip.
            //                $("#menuCategoryTimeRecipeGrid").jqxTooltip('open', pageX + 15, pageY + 5);
            //            }
            //            else {
            //                $("#menuCategoryTimeRecipeGrid").jqxTooltip('close');
            //            }
            //        }
            //        if (countTooltip == 8) countTooltip = 0;
            //    }
            //},
            columns: gridColumns
        });
        ...
        $("#menuCategoryTimeRecipeGrid").on("cellvaluechanged", function (event) {
            ...
            if (event.args.datafield == "EatingTimeID" || event.args.datafield == "MenuCorrectionTypeID") {
                ...
                var recipeFromGrid = recipeDA.records.filter(item => item.RecipeID == rowdata.RecipeID);
               // if (recipeFromGrid.length) rowdata.Netto = recipeFromGrid[0].Netto;
        ...
        function addNewRecipeRow() {
            ...
            var datarow = index == -1 ? generateMenuCategoryTimeRecipe(e.args.element.id)
                                  : generateMenuCategoryTimeRecipe(e.args.element.id, $('#menuCategoryTimeRecipeGrid').jqxGrid('getrowdata', index).EatingTimeID, $('#menuCategoryTimeRecipeGrid').jqxGrid('getrowdata', index).OrderNumber);
